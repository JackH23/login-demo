...
const normalizeComment = (comment) => {
  ...
  const normalizeId = (value) => (value?.toString ? value.toString() : value);
  ...
  return {
    ...
    updatedAt: normalizeDate(comment.updatedAt),
    replies: (comment.replies || []).map((reply) => ({
      ...reply,
      createdAt: normalizeDate(reply.createdAt),
      updatedAt: normalizeDate(reply.updatedAt),
    })),
  };
};
...
const attachAvatars = async (comments) => {
  ...
  return comments.map((comment) => {
    ...
    return {
      ...normalized,
      authorImage: imageMap[normalized.author],
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const [payload] = await attachAvatars([comment.toObject()]);

      req.app.get("io")?.emit("comment-created", { comment: payload });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    req.app.get("io")?.emit("comment-updated", { comment: withAvatars });
